...
// Removing from an empty queue should signal an error through 'queue_error'
...
// (i.e. A NULL queue cannot return the size, queue_error shoud be called.)
...
// Signal an error
void queue_error(int errorcode){
	if(errorcode==1){
		puts("Error: queue is empty");
		exit(0);
	}
	else if(errorcode==2){
		puts("Error: queue uninitialized");
		exit(0);
	}else if(errorcode==3){
		puts("Error: attempted enqueue on a full queue");
		exit(0);
	}else{
		puts("Error in queue");
		exit(0);		
